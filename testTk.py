...
class TkConfig(Config):
    ...
    def __init__(self):
        Config.__init__(
            self, argname=[
                'background', 'bg', 'borderwidth', 'bd', 'class', 'clormap', 'container', 'cursor',
                'height', 'highlightbackground', 'highlightcolor', 'padx', 'pady', 'relief', 'takefocus', 'visual', 'width', ])
...
if __name__ == "__main__":
    ...
    t = TLabel(text='hello')
    for i in range( 9):
        b = PButton(frame=w, text='%s' % i, command=w.test)
